...
namespace HcBimUtils
{
    public class Util
    {
        ...
        /// </summary>
        public static BoundingBoxXYZ GetBoundingBox(CurveLoop curveLoop)
        {
                                                             ...
                                                             out T ix, out T iy)
{
    ...
    return true;
        ...
        public static CurveArray CreateCurveArray(
            List<XYZ> pts)
        {
            int n = pts.Count;
        ...
        public static List<XYZ> ConvexHull(List<XYZ> points)
        {
            ...
            XYZ startPoint = points.MinBy2(p => p.X);
            ...
            while (isContinue)
            {
                ...
                for (int i = 0; i < convexHullPoints.Count; i++)
                {
                    ...
                    if (i == 0)
                    {
                        start = convexHullPoints[convexHullPoints.Count - 1];
                        mid = convexHullPoints[i];
                        end = convexHullPoints[i + 1];
                    }
                    else if (i == convexHullPoints.Count - 1)
                    {
                        start = convexHullPoints[i - 1];
                        mid = convexHullPoints[i];
                        end = convexHullPoints[0];
                    }
                    else
                    {
                        start = convexHullPoints[i - 1];
                    ...
                    if (!start.IsAlmostEqualTo(end))
                    {
                        ...
                        if (ln.Project(mid).Distance < 0.0001)
                        {
                            convexHullPoints.RemoveAt(i);
                            isContinue = true;
                            break;
                        }
                    }
        ...
        public static List<XYZ> ConvexHull(List<XYZ> points, XYZ faceNormal)
        {
            if (points != null && points.Count > 2)
            {
                XYZ startPoint = points.OrderByDescending(x => (x - points[0]).GetLength()).FirstOrDefault();
                XYZ refVector = startPoint - points[0];
                var convexHullPoints = new List<XYZ>();
                XYZ walkingPoint = startPoint;
                do
                {
                    ...
                    walkingPoint = points.MinBy2(p =>
                    {
                        if (!p.IsAlmostEqualTo(wp))
                        {
                            double angle = (p - wp).AngleOnPlaneTo(rv, faceNormal);
                            if (angle < 1e-10)
                                angle = 2 * Math.PI;
                            return angle;
                        }
        ...
        public static double CubicMeterToCubicFoot(double volume)
        {
            return volume / _cubicFootToCubicMeter;
        }

        public static double SquareMeterToSquareFoot(double volume)
        {
            return volume / _sqfToSqm;
        }
        ...
        public static TaskDialogResult TaskDialogShow(
              string title,
              string mainContent,
              TaskDialogIcon icon,
              TaskDialogResult defaultButton)
        {
            TaskDialog d = new TaskDialog(title);
            d.TitleAutoPrefix = false;
            d.MainContent = mainContent;
            d.MainIcon = icon;
            d.CommonButtons = TaskDialogCommonButtons.No | TaskDialogCommonButtons.Yes;
            d.DefaultButton = defaultButton;
            return d.Show();
        }
            ...
            bool unusedOnly)
        {
            ...
            if (unusedOnly)
            {
                return null == cm
                 ? null
                 : GetConnectorClosestTo(cm.UnusedConnectors, p);
            }
            else
            {
                return null == cm
    ...
    public static class JtElementIdExtensionMethods
    {
        ...
        {
            return ElementId.InvalidElementId == id || id == null;
        }
    ...
    "CreateUnbound";
        ...
        {
            System.Drawing.Rectangle value = WinForms.Screen.PrimaryScreen.Bounds;
            return value;
        }
